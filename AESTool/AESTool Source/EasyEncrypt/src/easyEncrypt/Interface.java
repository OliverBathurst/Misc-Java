...
public class Interface {
	...
	public static void main(String[] args) {			
		JFrame frame = new JFrame("AESTool");
		...
		JMenuItem genKeys = new JMenuItem("Generate key");
		JMenuItem printKey = new JMenuItem("Print key");
		JMenuItem exportKey = new JMenuItem("Export key");
		
		JMenu conf = new JMenu("Config");
		JMenuItem openpub = new JMenuItem("Open key file");
		conf.add(openpub);
		...
		openpub.addActionListener(new ActionListener() {     
			@Override
			public void actionPerformed(java.awt.event.ActionEvent arg0) {
				tryopenkey();				
			}
	...
	public static void exportkey(){
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {					
			...
			try {		
				...
				publicKeyOS.writeObject(Main.SecKey);	
				publicKeyOS.close();
